...
namespace showinfocontrol {
 ...
 public class ExRichTextBox : ControlRichTextBox.ExtRichTextBox {
  ...
  #region 插入链接
    ...
    //    Return num
    //End Function
     private string GetNeiMa(string linktext)
     {
         string str = "";
         int i;
         for (i = 0; i < linktext.Length; i++)
         {
             int num2 = (int)linktext[i];
             if (num2 > 160)
             {
                 char ch = linktext[i];
                 byte[] bs = Encoding.GetEncoding(936).GetBytes(ch.ToString());
                 string strtmp = str;
                 str = string.Concat(new string[] { strtmp, "\'", bs[0].ToString("x2"), "\'", bs[1].ToString("x2") });
     ...
     //    this.SelectedRtf= String.Concat(new string[] {"{\rtf1\ansicpg936 ", GetNeiMa(text), "\v #", GetNeiMa(hyperlink), "\v0}"});
    ...
    //    MyBase.SelectedRtf = String.Concat(New String() {"{\rtf1\ansicpg936 ", Me.GetNeiMa([text]), "\v #", Me.GetNeiMa(hyperlink), "\v0}"})
